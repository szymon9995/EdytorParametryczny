...
public class TestClass {
	...
	public void Run() throws URISyntaxException,ExportException
	{
        ...
        URI start = hrefGraph
            .vertexSet().stream().filter(new Predicate<URI>() {
				@Override
				public boolean test(URI uri) {
					return uri.getHost().equals("www.jgrapht.org");
				}
			}).findAny()
            .get();
        ...
        throws ExportException
    {
        ...
        DOTExporter<URI, DefaultEdge> exporter =
            new DOTExporter<>(new Function<URI, String>() {
				@Override
				public String apply(URI v) {
					return v.getHost().replace('.', '_');
				}
			});
        exporter.setVertexAttributeProvider(new Function<URI, Map<String, Attribute>>() {
			@Override
			public Map<String, Attribute> apply(URI v) {
