...
public class IFPolygon extends IFShape
{
	public IFPolygon(DrawComponent selected, JPanel editor) {
	...
	public void UpdateData()
	{
			...
			String n = ""+tmp.getNSize();
			posN = new JTextField(n,10);
			...
			objectEditor.add(NLabel);
			objectEditor.add(posN);
			...
			posN.addActionListener(new ActionListener()
			{

				@Override
				public void actionPerformed(ActionEvent e) {
					
					if(posN.getText()!=null)
					{
						int nPos =Integer.parseInt(posN.getText());
						if(nPos>-1)
						{
							tmp.setNSize(nPos);
							int n_now = tmp.getNSize();
							posN.setText(n_now+"");
							EdytorData.Repaint();
						}
					}
